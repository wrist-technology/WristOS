...
void abcsp_crc_update(uint16 *crc, const uint8 *block, int len)
{
	uint16 reg;
	...
	do {
		reg = reg>>8 ^ crc_table[(reg & 255) ^ *block++];
	} while(--len);
...
uint16 abcsp_crc_block(const uint8 *block, int len)
{
	uint16 crc;

	abcsp_crc_init(&crc);
	abcsp_crc_update(&crc, block, len);
	return abcsp_crc_reverse(crc);
}
