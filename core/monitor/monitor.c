...
static int monitorLua (int argc, portCHAR **argv);
#ifdef CFG_MD
...
{
  { "help",     0,  0, CMDTYPE_FUNCTION,  { monitorHelp        }, "This help list",                 "'help' has no parameters" },
  { "lua",     0,  0, CMDTYPE_FUNCTION,  { monitorLua        }, "Lua interpreter",                 "'lua' has no parameters" },
#ifdef CFG_ABORT
...
static int monitorLua (int argc __attribute__ ((unused)), portCHAR **argv __attribute__ ((unused)))
{
  ...
  for (;;)
  {
    ...
    if ((l = argsGetLine (fd, buffer, sizeof (buffer), PROMPT)))
    {
      ...
      int error = luaL_loadbuffer(L, buffer, strlen(buffer), "line") || lua_pcall(L, 0, 0, 0); 
      if (error) {
...
static int monitorFileDf (int argc __attribute__ ((unused)), portCHAR **argv __attribute__ ((unused)))
{
  ...
  printf ("\n%u files, %u bytes.\n%u folders.\n"
      ...
      (fs->max_clust - 2) * fs->csize * 512, p2 * fs->csize * 512
      );
...
static int monitorFileLs (int argc, portCHAR **argv)
{
  ...
  if (f_getfree (path, (U32 *) &size, &fs) == FR_OK)
    printf (", %10uK bytes free", size * fs->csize / 2);
...
{
  printf ("DynawaTCH, Version " __VERSION ", " __DATE__ " " __TIME__ "\n");
  printf ("Copyright (c) 2008, J.C. Wren\n");
  printf ("Copyright (c) 2009, Dynawa\n");
...
{
  return xTaskCreate (vMonitorTask,  (const signed portCHAR * const) "Monitor", 8192, NULL, (tskIDLE_PRIORITY + 3), &taskHandles [TASKHANDLE_MONITOR]);
}
