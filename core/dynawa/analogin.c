...
// dynawa
#define ANALOGIN_0_IO IO_PA17
#define ANALOGIN_1_IO IO_PA18
#define ANALOGIN_2_IO IO_PA19
#define ANALOGIN_3_IO IO_PA20
...
// statics
static AnalogIn_Manager analogin_manager;
extern AnalogIn_Manager *p_analogin_manager;
...
void AnalogIn_init(AnalogIn *analogin,  int channel )
{
    ...
    analogin->index = channel;
    if(!analogin_manager.activeChannels)
    {
        AnalogIn_managerInit();
        analogin_manager.activeChannels |= (1 << analogin->index); // mark it as used
    }
    ...
    {
        Io pin;
        Io_init( &pin, AnalogIn_getIo( analogin->index ), IO_GPIO, INPUT );
        Io_setPullup( &pin, false );
    }
}
...
\endcode
*/
bool AnalogIn_multi( int values[] ) // static
{
    if(!analogin_manager.activeChannels)
        AnalogIn_managerInit();
    ...
    analogin_manager.waitingForMulti = true;
...
int AnalogIn_valueWait(AnalogIn *analogin)
{
    ...
    if (AT91C_BASE_ADC->ADC_SR & AT91C_ADC_DRDY) {
        return -1;
    }
...
/*
    ...
    return AT91C_BASE_ADC->ADC_CDR7;
*/
}
...
// Petr
    AT91C_BASE_ADC->ADC_CR = AT91C_ADC_SWRST;     // reset to clear out previous settings
    AT91C_BASE_ADC->ADC_CR = 0;     // reset to clear out previous settings
    AT91C_BASE_ADC->ADC_CR = 0;     // reset to clear out previous settings
*/
